...
fn main() {
	...
	let features = dataset.headers.clone().into_iter().filter(|f| f!="class").collect::<Vec<_>>();
	...
	for feature in features.iter() {
		*relevances.get_mut(feature).unwrap() = dataset.mutual_info(feature, "class").unwrap();
	}
	...
	while !remaining_features.is_empty(){
		...
		for feature in remaining_features.iter(){
			*redundances.get_mut(feature).unwrap()+= dataset.mutual_info(feature, &selected_features.last().unwrap().0).unwrap();
		};
